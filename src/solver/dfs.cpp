...
bool solveMaze(int rows, int cols, int* maze, int* path,pair<int, int> P0,pair<int, int> P1) {
...
void mazesolver_child_dfs(int rows,int cols,pair<int,int> p0,pair<int,int> p1,int* maze){
    
    int* path = (int*)malloc(rows*cols*sizeof(int));
    for(int i=0;i<rows*cols;i++)path[i]=0;

    bool solvable = solveMaze(rows,cols,maze,path,p0,p1);
    if(!solvable){
...
void mazesolver_dfs(int n,int m,string filename){
    int num_procs,rank;
    ...
    mazesolver_child_dfs(t,m,make_pair(0,start_point),make_pair(t-1,end_point),partial_maze);
    if(rank != 3){
